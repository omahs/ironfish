...
pub struct ValueBalances {
    values: HashMap<AssetIdentifier, i128>,
}
...
impl ValueBalances {
    ...
    pub fn add(&mut self, asset_identifier: &AssetIdentifier, value: i128) {
        let current_value = self.values.entry(*asset_identifier).or_insert(0);
        *current_value += value
    }

    pub fn subtract(&mut self, asset_identifier: &AssetIdentifier, value: i128) {
        let current_value = self.values.entry(*asset_identifier).or_insert(0);
        *current_value -= value
    }

    pub fn iter(&self) -> hash_map::Iter<AssetIdentifier, i128> {
        self.values.iter()
    }

    pub fn fee(&self) -> i64 {
        *self.values.get(&NATIVE_ASSET).unwrap() as i64
    }
}
